...
import shared/contacts/contact.{
  type ContactInput, type PipelineStage, ContactInput, ContactStage,
  CustomerStage, LeadStage, OpportunityStage,
}
...
pub fn build(factory: ContactFactory) -> ContactInput {
  ContactInput(
...
pub fn build_lead() -> ContactInput {
  new() |> with_stage(LeadStage) |> build()
}

pub fn build_customer() -> ContactInput {
  new() |> with_stage(CustomerStage) |> build()
}

pub fn build_opportunity() -> ContactInput {
  new() |> with_stage(OpportunityStage) |> build()
}

pub fn build_contact_stage() -> ContactInput {
