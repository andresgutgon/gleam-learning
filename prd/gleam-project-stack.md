...
### Theme: CSS variables (shadcn pattern)

`assets/theme.css` defines all colors as HSL custom properties for light + dark
mode. Tailwind `@theme` block maps them to utilities (`bg-background`,
`text-foreground`, `border-border`, etc.). Switching the entire app's look is
just changing HSL values.
...
│       └── theme.css            # HSL CSS variables (light + dark)
├── priv/
